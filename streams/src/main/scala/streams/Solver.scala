...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors filter{case (b, _) => !explored.contains(b)}
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      ...
      case (block, history) #:: xs =>
        ...
        (block, history) #:: from(xs ++ newNeighbors, explored ++ newNeighbors.flatMap{case (block, _) => Set(block)})
    }
  }
