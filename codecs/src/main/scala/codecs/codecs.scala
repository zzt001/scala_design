...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(x) => if (x.isValidInt) Option(x.intValue) else None
      case _ => None
    }
  /** A decoder for `String` values */
  implicit val strDecoder: Decoder[String] =
    Decoder.fromFunction {
      case Json.Str(s) => Option(s)
      case _ => None
    }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) =>
        val optionValues = arr.map(decoder.decode)
        if (optionValues.forall({
          case Some(_) => true
          case _ => false
        })) Option(optionValues.map({case Some(x) => x}))
        else {
          None
        }
      case _ => None
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(obj) => if (obj.contains(name)) decoder.decode(obj(name)) else None
      case _ => None
    }
